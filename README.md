...
| Автопереключение | Переключает канал вниз при окончании текущего таймслота (эфира) |
| Пропуск снега | Автоматически пропускает каналы с `snowEpisodeObject` |
| Отключить деградацию сигнала | Раз в 120 с сбрасывает `signalLossInProgress` в `false` |
| Окно настроек (F10) | Чекбоксы режимов, слайдер интервала 1–60 мин, статусы |
...
2. Скачайте `BlippoChannelHopper.dll` из релиза **v1.1.0**.
...
- **Переключение по таймеру** — hop каждые N минут
- **Автопереключение** — hop в конце таймслота
- **Пропуск снега** — сразу уходить со snow-каналов
- **Отключить деградацию сигнала** — сброс флага деградации каждые 120 с
- слайдер 1–60 мин, кнопки **-1 min** / **+1 min**, статусы режимов

Режимы независимы и могут работать вместе.
